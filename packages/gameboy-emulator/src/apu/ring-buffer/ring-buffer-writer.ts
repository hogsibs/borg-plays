...
export class RingBufferWriter<BufferClassType extends ArrayBufferClassTypes> {
  ...
  private static SizeOfPointers =
    RingBufferWriter.WritePointerByteSize +
    RingBufferWriter.ReadPointerByteSize;
  ...
  static GetSharedArrayBufferForCapacity(
    capacity: number,
    bufferType: ArrayBufferClassTypes
  ): SharedArrayBuffer {
    const capacityWithEmptySlot = capacity + 1;
    const bytes =
      capacityWithEmptySlot * bufferType.BYTES_PER_ELEMENT +
      RingBufferWriter.SizeOfPointers;
    return new SharedArrayBuffer(bytes);
  }

  constructor(
    sharedArrayBuffer: SharedArrayBuffer,
    BufferClass: BufferClassType
  ) {
    this.capacity =
      (sharedArrayBuffer.byteLength - RingBufferWriter.SizeOfPointers) /
      BufferClass.BYTES_PER_ELEMENT;
    ...
    this.storage = new BufferClass(
      this.sharedBuffer,
      8,
      this.capacity
    ) as InstanceType<BufferClassType>;
  }
  ...
  push(elements: InstanceType<BufferClassType>) {
    ...
    const sizeUpToEndOfArray = Math.min(
      this.capacity - writePosition,
      howManyToWrite
    );
    ...
    this.copy(
      ...
      this.storage,
      0,
      sizeFromStartOfArrayOrZero
    );

    const writePointerPositionAfterWrite =
      (writePosition + howManyToWrite) % this.capacity;
  ...
  private copy(
    input: InstanceType<BufferClassType>,
    offsetInput: number,
    output: InstanceType<BufferClassType>,
    offsetOutput: number,
    size: number
  ) {

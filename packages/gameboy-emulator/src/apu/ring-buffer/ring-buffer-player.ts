...
export class RingBufferPlayer {
  ...
  private ringBufferPlayerNode?: RemoteAudioWorkletNode;
  ...
  /**
   * @constructor
   * @param {RemoteAudioContext} audioContext - Audio context to use for playing audio
  ...
  constructor(audioContext: RemoteAudioContext, sampleBufferLength: number) {
    ...
    const parameterArrayBufferSize =
      parameterIndexByteSize + parameterValueByteSize;
    ...
    audioContext.audioWorklet.addModule(this.getModuleFile()).then(() => {
      const audioQueueSharedBuffer =
        RingBufferWriter.GetSharedArrayBufferForCapacity(
          audioContext.sampleRate / 20,
          Float32Array
        );
      this.audioRingBufferWriter = new RingBufferWriter(
        audioQueueSharedBuffer,
        Float32Array
      );

      const parameterQueueSharedBuffer =
        RingBufferWriter.GetSharedArrayBufferForCapacity(32, Uint8Array);
      this.parameterRingBufferWriter = new RingBufferWriter(
        parameterQueueSharedBuffer,
        Uint8Array
      );

      this.ringBufferPlayerNode = new RemoteAudioWorkletNode(
        audioContext,
  ...
  writeParameter(index: number, value: number) {
    ...
    if (
      !this.parameterRingBufferWriter ||
      this.parameterRingBufferWriter.availableWrite() <
        this.parameterRawStorage.byteLength
    ) {
      return false;
    }
    return (
      this.parameterRingBufferWriter &&
      this.parameterRingBufferWriter.push(this.parameterByteArray) ===
        this.parameterRawStorage.byteLength
    );
  }

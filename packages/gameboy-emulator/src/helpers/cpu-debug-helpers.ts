...
export function updateInstructionCache(
  instruction: string,
  programCounterVal: number,
  af: number,
  bc: number,
  de: number,
  hl: number,
  ...
  instructionCache.unshift(
    `${programCounterVal.toString(16)}: ${instruction} --AF: ${af
      .toString(16)
      .padStart(4, "0")} --BC: ${bc.toString(16).padStart(4, "0")} --DE: ${de
      .toString(16)
      .padStart(4, "0")} --HL: ${hl
      .toString(16)
      .padStart(4, "0")} --SP: ${sp.toString(16)}`
  );

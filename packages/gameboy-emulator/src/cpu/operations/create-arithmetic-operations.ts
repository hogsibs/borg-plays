...
export function createArithmeticOperations(this: CPU) {
  ...
  function addCarryAndSetFlags(
    accumulatorVal: number,
    toAdd: number,
    carryFlagValue: number
  ) {
    const newValue = (accumulatorVal + toAdd + carryFlagValue) & 0xff;
    registers.flags.isResultZero = newValue === 0;
    registers.flags.isHalfCarry =
      (accumulatorVal & 0x0f) + (toAdd & 0x0f) + carryFlagValue > 0xf;
    registers.flags.isSubtraction = false;
  ...
  this.registers.baseRegisters.forEach((register) => {
    this.addOperation({
      ...
      byteLength: 1,
      execute() {
        registers.A.value = addCarryAndSetFlags(
          registers.A.value,
          register.value,
          registers.flags.CY & 0xff
        );
      },
    });
  });
  ...
  registers.registerPairs
    ...
    .forEach((registerPair) => {
      this.addOperation({
        ...
        byteLength: 1,
        execute() {
          registers.HL.value = add16BitAndSetFlags(
            registers.HL.value,
            registerPair.value
          );
        },
  ...
  this.registers.baseRegisters.forEach((register) => {
    this.addOperation({
      ...
      execute() {
        registers.A.value = subtractAndSetFlags(
          registers.A.value,
          register.value
        );
      },
  ...
  function subtractCarryAndSetFlags(
    accumulatorVal: number,
    toAdd: number,
    carryFlagValue: number
  ) {
    ...
    registers.flags.isResultZero = newValue === 0;
    registers.flags.isHalfCarry =
      (accumulatorVal & 0x0f) - (toAdd & 0x0f) - carryFlagValue < 0;
    registers.flags.isSubtraction = true;
    registers.flags.isCarry = newValue > accumulatorVal - carryFlagValue;
  ...
  this.registers.baseRegisters.forEach((register) => {
    this.addOperation({
      ...
      execute() {
        registers.A.value = subtractCarryAndSetFlags(
          registers.A.value,
          register.value,
          registers.flags.CY
        );
